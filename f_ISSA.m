...
function [mbest,stdbest,sem,mFEs,SR,pos,cg_curve] = f_ISSA(N,Max_iter,RUNS,M,mat_HOS,SNR,Nt,lMC)
...
while l<Max_iter+1
    ...
    for i=1:size(SalpPositions,1)
        ...
        elseif i>N/2 && i<N+1
           r=rand();
           if(r<0.5)
            point1=c1*SalpPositions(:,i-1);           
            point2=SalpPositions(:,i);
            SalpPositions(:,i)=c*(point2+point1)/2; % % Eq. (3.4) in the paper
           else
            SalpPositions(:,i)=(ub(:)+lb(:))- c1*c*SalpPositions(:,i);
           end
        end
